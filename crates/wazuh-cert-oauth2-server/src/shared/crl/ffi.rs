...
pub(crate) unsafe fn create_crl() -> Result<*mut ffi::X509_CRL> {
    unsafe {
        debug!("Creating new X509_CRL");
        let crl = ffi::X509_CRL_new();
        if crl.is_null() {
            anyhow::bail!(AppError::CrlFfi {
                func: "X509_CRL_new"
            });
        }
...
pub(crate) unsafe fn set_times_now_and_next(crl: *mut ffi::X509_CRL) -> Result<()> {
    unsafe {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs() as i64;
        debug!("Setting CRL lastUpdate={} nextUpdate={} (seconds since epoch)", now, now + 86400);
...
pub(crate) unsafe fn encode_der_and_free(crl: *mut ffi::X509_CRL) -> Result<Vec<u8>> {
    unsafe {
